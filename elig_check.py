...
with urllib.request.urlopen(req, timeout=40, context=ctx) as resp:
    print("ELIGIBILITY:", resp.status)
    print(resp.read().decode()[:500])

# check the persisted eligibility row
bash = r"""
DB=SchoolManagementSystem
docker exec sms-postgres psql -U sms_user -d $DB -x -c "SELECT set_config('app.tenant_id','11111111-1111-1111-1111-111111111111',false); select * from \"StudentCertificateEligibilities\""
"""
rc = subprocess.run(["ssh", "-o", "BatchMode=yes", "sms_admin@192.168.110.161", bash],
