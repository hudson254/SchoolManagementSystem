...
import { Box, Paper, Typography, TextField, Button, Grid, MenuItem, CircularProgress, Alert, Snackbar } from "@mui/material";
...
export const AddUserPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ firstName: "", lastName: "", email: "", phoneNumber: "", password: "", role: "User" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await userService.createUser(formData); setSuccess(true); setTimeout(() => navigate("/users"), 1500); }
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Paper sx={{ p: 4, maxWidth: 700, mx: "auto" }}>
        <Typography variant="h5" fontWeight={600} sx={{ mb: 3 }}>Add New User</Typography>
        ...
        <Box component="form" onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}><TextField fullWidth label="Role" name="role" select value={formData.role} onChange={handleChange} required>
          ...
          <Box sx={{ mt: 4, display: "flex", gap: 2, justifyContent: "flex-end" }}>
            ...
            <Button type="submit" variant="contained" disabled={loading}>{loading ? <CircularProgress size={24} /> : "Create User"}</Button>
          </Box>
        </Box>
      </Paper>
      <Snackbar open={success} message="User created successfully" autoHideDuration={3000} />
    </Box>
  );
};
