...
namespace SMS.Application.Features.Accommodation.Queries
{
    ...
    public class GetOccupancyReportQueryHandler : IRequestHandler<GetOccupancyReportQuery, OccupancyReportDto>
    {
        ...
        {
            var allRooms = await _accommodationRepository.GetRoomsByBuildingAsync(request.BuildingId?.ToString() ?? "", cancellationToken);
            ...
            var buildingGroups = allRooms
                .GroupBy(r => r.Block?.Building ?? r.Block?.Name ?? "Unknown")
                .Select(g => new
                {
                    BuildingName = g.Key,
                    Rooms = g.ToList()
