...
namespace SMS.Application.Features.Assignments.Queries
{
    ...
    public class GetAssignmentsQueryHandler : IRequestHandler<GetAssignmentsQuery, PagedResult<AssignmentDto>>
    {
        ...
        {
            var assignments = await _assignmentRepository.GetAssignmentsAsync(request.Page, request.PageSize, request.UnitId, request.LecturerId, cancellationToken);

            var totalCount = await _assignmentRepository.CountAssignmentsAsync(request.UnitId, request.LecturerId, cancellationToken);
